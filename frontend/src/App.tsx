      ...
      <main className="relative z-10 max-w-xl mx-auto px-4 py-8">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800/50 border border-white/10 rounded-2xl p-6 mb-6 shadow-2xl">
          ...
          : <button onClick={() => writeContract({ address: ADDR, abi: ABI, functionName: "logSwap", args: [from, to, parseUnits(amount || "0", 6), note] })} disabled={isLoading || !amount || Number(amount) <= 0}
              ...
              {isLoading ? <><svg className="spinner w-4 h-4 border-2 border-current border-t-transparent rounded-full" viewBox="0 0 24 24" />{isPending ? "Confirm…" : "Logging…"}</> : `🔄 Log ${from} → ${to}`}
            </button>}
        ...
        <div className="space-y-2">
          ...
          {list.map((s: any, i: number) => (
            <div key={i} className="bg-slate-900/70 border border-white/8 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-[#6366f1] font-bold text-sm">{s.fromToken}</span>
                  <span className="text-slate-500">→</span>
                  <span className="text-white font-bold text-sm">{s.toToken}</span>
                  <span className="text-slate-500 text-xs ml-auto">{timeAgo(s.timestamp)}</span>
                </div>
                <p className="text-slate-600 text-xs font-mono">{s.user.slice(0,6)}…{s.user.slice(-4)}{s.note ? ` · ${s.note}` : ""}</p>
              </div>
            </div>
          ))}
        </div>
        <footer className="mt-12 text-center text-xs text-slate-600">
          ...
          <p className="mt-1 text-slate-700">Note: This logs swap intents on-chain for Arc DeFi activity tracking</p>
        </footer>
      </main>
    </div>
  );
